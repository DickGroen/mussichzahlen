...
function issueBlocks(text = "") {
  // Renders ISSUES as: bold heading + normal paragraph pairs
  // Claude writes issues as: "Heading\nExplanation text" per issue
  const lines = String(text || "")
    .split("\n")
  ...
  while (i < lines.length) {
    ...
    // Heuristic: short line (< 80 chars) followed by longer line = heading + body
    if (line.length < 80 && nextLine.length > 40) {
      // heading
      out += `{\\pard\\sb320\\sa80\\f1\\fs22\\b\\cf1 ${rtfEscape(sanitizeLegalTone(line))}\\b0\\cf0\\par}\n`;
      // body
      out += `{\\pard\\sb0\\sa160\\f1\\fs22 ${rtfEscape(sanitizeLegalTone(nextLine))}\\par}\n`;
      i += 2;
    } else {
      // plain paragraph
      out += `{\\pard\\sb80\\sa160\\f1\\fs22 ${rtfEscape(sanitizeLegalTone(line))}\\par}\n`;
      i++;
    }
  }
  return out;
...
function formatAmount(triage = {}) {
  ...
  return `${detectCurrency(triage)}${formatted}`;
}
