...
window.startGratisUpload = async function() {
  ...
  try {
    ...
    if (btn) {
      btn.disabled = false;
      btn.textContent = 'Kostenlos einordnen lassen';
    }
  }
};
...
function getFallbackTeaser(risk) {
  if (risk === 'high') {
    return 'Mehrere Punkte in diesem Schreiben könnten vor einer Zahlung oder Reaktion eine genauere Klärung wert sein — eine ausführlichere Prüfung zeigt, welche das sind.';
  }

  if (risk === 'medium') {
    return 'Einige Punkte in diesem Schreiben könnten vor einer Entscheidung eine Bestätigung wert sein.';
  }

  return 'Das Schreiben wirkt vergleichsweise üblich; eine ausführlichere Prüfung kann die Details bestätigen, bevor Sie reagieren.';
}
  ...
  teaser.innerHTML = `
    <div class="offer-card teaser-card" style="border-color:var(--green);background:#f0fdf4;max-width:620px;margin:0 auto;">
      <div style="font-size:1.1rem;font-weight:700;color:#14532d;margin-bottom:12px;">
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:-3px;"><polyline points="20 6 9 17 4 12"></polyline></svg> Ihr Bescheid ist eingegangen.
      </div>
...
function ctaText() {
  return `Vollständige Analyse + Antwortvorlage erhalten — €${PRICE} →`;
}
