...
// Null-guarded: only exist on index.html, not danke.html
...
function showTeaser(triage) {
  ...
  const bescheidTyp  = triage.bescheid_typ || null;
  const risk         = triage.risk || 'medium';

  const isPrivat = bescheidTyp === 'privat';
  document.getElementById('teaser-company').textContent = amount
    ? `${isPrivat ? 'Privates Parkschreiben' : 'Bu\u00dfgeldbescheid'} \u00fcber ${amount} \u2014 m\u00f6gliche Ansatzpunkte identifiziert`
    : sender
      ? `${sender} \u2014 m\u00f6gliche Ansatzpunkte identifiziert`
      : 'Wir haben m\u00f6gliche Ansatzpunkte in deinem Bescheid identifiziert';

  const riskMsg = {
    high:   '\uD83D\uDD34 Deutliche Ansatzpunkte erkannt \u2014 Einspruch empfohlen.',
    medium: '\uD83D\uDFE0 M\u00f6gliche Ansatzpunkte vorhanden \u2014 eine vollst\u00e4ndige Pr\u00fcfung gibt Gewissheit.',
    low:    '\uD83D\uDFE1 Begrenzte Ansatzpunkte \u2014 aber eine Pr\u00fcfung kann trotzdem lohnen.'
  };
  document.getElementById('teaser-sub').textContent = riskMsg[risk] || '';

  const locked = document.getElementById('teaser-locked-text');
  if (locked) {
    locked.innerHTML = `<strong>Vollst\u00e4ndige Analyse + Einspruchsschreiben nach Zahlung</strong>
  ...
  if (modalCopy) {
    modalCopy.textContent = amount
      ? `Wir haben m\u00f6gliche Ansatzpunkte f\u00fcr deinen Bescheid \u00fcber ${amount} identifiziert \u2014 vollst\u00e4ndige Analyse nach der Zahlung.`
      : 'Wir haben m\u00f6gliche Ansatzpunkte in deinem Bescheid identifiziert \u2014 vollst\u00e4ndige Analyse nach der Zahlung.';
  }
  ...
  try {
    ...
    document.querySelector('.thankyou-card').innerHTML = `
      <div class="success-screen">
        ...
        <p>Wir analysieren deinen Bescheid und senden dir die vollst\u00e4ndige Analyse sowie das fertige Einspruchsschreiben an <strong>${esc(email)}</strong> innerhalb von 24\u00a0Stunden.</p>
